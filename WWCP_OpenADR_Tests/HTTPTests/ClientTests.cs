...
namespace cloud.charging.open.protocols.OpenADRv3.tests.HTTP
{
    ...
    /// <summary>
    /// OpenADR client tests.
    /// </summary>
    [TestFixture]
    public class ClientTests : AHTTPTests
    {
        ...
        public async Task Test1()
        {
            ...
            if (testOpenADR1 is not null)
            {
                ...
                var client = new OpenADRClient(
                             URL.Parse($"http://localhost:{testOpenADR1.HTTPExtAPI?.HTTPServer.IPPorts.First()}/OpenADR/"),
                             DNSClient: testOpenADR1.DNSClient
                         );

                var programs = await client.GetPrograms();
